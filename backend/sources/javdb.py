...
class JavdbSource:
    ...
    def __init__(self, session=None, delay=0.6): self.session=session or requests.Session(); self.delay=delay
    def request(self,url):
        for attempt in range(3):
            try:
                response=self.session.get(url,headers=self.headers,timeout=20); response.raise_for_status(); response.encoding='utf-8'; return response.text
    ...
    def fetch_movie(self,url):
        soup=BeautifulSoup(self.request(url),'lxml'); text=' '.join(soup.stripped_strings)
    ...
    def fetch_magnets(self,url):
        soup=BeautifulSoup(self.request(url),'lxml'); out=[]
