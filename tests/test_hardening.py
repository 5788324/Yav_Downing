...
class ShutdownFrontendTests(unittest.TestCase):
    def test_shutdown_ui_contract(self):
        ...
        self.assertIn('id="shutdownOverlay"', html)
        self.assertIn('id="confirmShutdown"', html)
        self.assertIn('id="cancelShutdown"', html)
        self.assertIn("closeShutdownConfirm", script)
        self.assertIn("$('#confirmShutdown')?.addEventListener('click'", script)
